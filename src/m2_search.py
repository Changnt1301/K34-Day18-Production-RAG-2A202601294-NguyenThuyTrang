...
from config import (QDRANT_HOST, QDRANT_PORT, QDRANT_URL, QDRANT_API_KEY, COLLECTION_NAME,
                    EMBEDDING_MODEL, EMBEDDING_DIM, BM25_TOP_K, DENSE_TOP_K, HYBRID_TOP_K)
...
def segment_vietnamese(text: str) -> str:
    ...
    # split(" ") → "nghỉ_phép" thành 1 token, nhưng query "nghỉ phép" thành 2 token
...
class DenseSearch:
    ...
    def search(self, query: str, top_k: int = DENSE_TOP_K, collection: str = COLLECTION_NAME) -> list[SearchResult]:
        ...
        # ⚠️ qdrant-client >= 1.9 dùng query_points(), KHÔNG phải search().
