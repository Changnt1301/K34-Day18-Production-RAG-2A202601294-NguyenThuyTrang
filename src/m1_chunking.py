...
def _group_by_size(paragraphs: list[str], size: int) -> list[str]:
    """Gộp paragraphs liên tiếp thành các nhóm <= size ký tự (best-effort).
    Paragraph nào tự nó đã lớn hơn size sẽ bị cắt cứng theo size."""
    groups = []
    current = ""
...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    for parent_text in _group_by_size(paragraphs, parent_size):
        pid = f"parent_{len(parents)}"
        parents.append(Chunk(text=parent_text, metadata={**metadata, "chunk_type": "parent", "parent_id": pid}))

        sub_paragraphs = [p.strip() for p in parent_text.split("\n\n") if p.strip()] or [parent_text]
        for child_text in _group_by_size(sub_paragraphs, child_size):
...
def chunk_structure_aware(text: str, metadata: dict | None = None) -> list[Chunk]:
    ...
    def _flush():
        if not current_header and not current_content.strip():
            return
        ...
        chunks.append(Chunk(text=chunk_text, metadata={**metadata, "section": section,
